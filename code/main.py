...
# tf.app.flags.DEFINE_integer("update_lr_freq", 20, 'update learning rate frequency')
...
def evaluate(model, sess, dataset):
    ...
    while ed < len(dataset):
        ...
        outputs = sess.run([model.loss, model.accuracy], {model.texts: batch_data['texts'], model.texts_length: batch_data['texts_length'],
                            model.labels: batch_data['labels'], model.output_keep_prob: 1.0})
        loss += outputs[0]
        accuracy += outputs[1]
...
def inference(model, sess, dataset):
    ...
    while ed < len(dataset):
        ...
        outputs = sess.run([model.predict_labels], {model.texts: batch_data['texts'], model.texts_length: batch_data['texts_length'], model.output_keep_prob: 1.0})
...
with tf.Session(config=config) as sess:
    if FLAGS.is_train:
        ...
        while model.epoch.eval() < FLAGS.epoch:
            ...
            print("epoch %d learning rate %.10f epoch-time %.4f loss %.8f accuracy [%.8f]" % (epoch, cur_lr, epoch_time, loss, accuracy))
            # uncomment below if want to slow down lr
            # if epoch == 50:
                # sess.run([model.learning_rate_update_op])
        ...
        with open(FLAGS.time_log_path, 'a') as fp:
